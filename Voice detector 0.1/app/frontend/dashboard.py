...
# --- CONFIG ---
API_URL = "https://voxveritas-api.onrender.com/api/v1/detect"
...
st.markdown("""
    ...
    /* 1. Global Background & Sidebar Reset */
    .stApp {
    ...
    .main-header {
        ...
        /* Subtle glow that doesn't blur letters */
            ...
            try:
                ...
                if r.status_code == 200:
                    ...
                    if res['prediction'] == "AI_GENERATED":
                        st.error("### 🛑 ALERT: AI SYNTHETIC DETECTED")
                    else:
                        st.success("### ✅ VERIFIED: HUMAN AUTHENTIC")
                    
                    st.metric("Detection Confidence", f"{round(res['confidence']*100, 2)}%")
                    st.rerun()
                else:
                    st.error("API Rejected Signal. Check Audio Length.")
            except:
                st.error("🚨 Backend Offline. Run main.py on Port 8000.")
...
st.markdown("<p style='text-align:center; color:#555; font-size:0.7rem;'>Build v5.0 Platinum Stable | Developed by Blaze</p>", unsafe_allow_html=True)